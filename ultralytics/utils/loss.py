...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        target_labels = target_scores[fg_mask].argmax(-1)
        srif_weights = torch.ones_like(target_labels, dtype=pred_bboxes.dtype)
        srif_weights[target_labels < 11] = 8.5
        srif_weights[target_labels == 11] = 1.0
        final_weight = weight * srif_weights.unsqueeze(-1)
        weighted_norm_factor = torch.clamp(final_weight.sum(), min=1.0)
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
        loss_iou = ((1.0 - iou) * final_weight).sum() / weighted_norm_factor

        if self.dfl_loss:
            target_ltrb = bbox2dist(anchor_points, target_bboxes, self.dfl_loss.reg_max - 1)
            loss_dfl_raw = self.dfl_loss(pred_dist[fg_mask].view(-1, self.dfl_loss.reg_max), target_ltrb[fg_mask])
            loss_dfl = (loss_dfl_raw * final_weight).sum() / weighted_norm_factor
        else:
            ...
            loss_dfl_raw = F.l1_loss(pred_dist[fg_mask], target_ltrb[fg_mask], reduction="none").mean(-1, keepdim=True)
            loss_dfl = (loss_dfl_raw * final_weight).sum() / weighted_norm_factor
...
class v8DetectionLoss:
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        dtype = pred_scores.dtype
        cls_weights = torch.ones(self.nc, device=device, dtype=dtype)
        cls_weights[0:11] = 8.5
        cls_weights[11] = 1.0

        cls_loss_raw = self.bce(pred_scores, target_scores.to(dtype))
        cls_loss_weighted = cls_loss_raw * cls_weights

        weighted_target_scores_sum = (target_scores.to(dtype) * cls_weights).sum()

        norm_factor = torch.clamp(weighted_target_scores_sum, min=1.0)
        loss[1] = cls_loss_weighted.sum() / norm_factor
