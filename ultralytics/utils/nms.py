...
def non_max_suppression(
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        if rotated:
            ...
            mask_char = (cls_ids != WATER_ID)
            i_char = torch.tensor([], dtype=torch.int64, device=boxes.device)
            if mask_char.any():
                idx_local_char = torch.where(mask_char)[0]
                keep_char = torchvision.ops.nms(boxes[mask_char], scores[mask_char], 0.45)
                i_char = idx_local_char[keep_char]  

            
            mask_water = (cls_ids == WATER_ID)
            i_water = torch.tensor([], dtype=torch.int64, device=boxes.device)
            if mask_water.any():
                idx_local_water = torch.where(mask_water)[0]
                keep_water = torchvision.ops.nms(boxes[mask_water], scores[mask_water], 0.85)
                i_water = idx_local_water[keep_water]  

            
            i = torch.cat([i_char, i_water])
            if i.numel() > 0:
