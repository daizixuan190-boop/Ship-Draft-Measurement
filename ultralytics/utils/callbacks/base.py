...
def add_integration_callbacks(instance):
    ...
    try:
        ...
        callbacks_list.append(platform_cb)
    except Exception:
        pass

    # Load training callbacks
    if "Trainer" in instance.__class__.__name__:
        for module_name in ("clearml", "comet", "dvc", "mlflow", "neptune", "raytune", "tensorboard", "wb"):
            try:
                module = __import__(f"ultralytics.utils.callbacks.{module_name}", fromlist=["callbacks"])
                callbacks_list.append(module.callbacks)
            except Exception:
                pass
